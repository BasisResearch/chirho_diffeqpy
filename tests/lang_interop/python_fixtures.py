...
single_op_scalar_dunder_argskwargs_fns = [
    ...
    (
        lambda x, y: x + y,
        (
            ((1.0, 2.0), dict()),  # two args
            ((-1.0,), dict(y=1.0)),  # one arg one kwarg
            (tuple(), dict(x=-3.0, y=0.0)),  # two kwargs
        ),
    ),
    ...
    (
        ...
        (
            ((1.0,), dict()),  # one arg
            ((-1.0,), dict(y=1.0)),  # one arg one kwarg
            (tuple(), dict(x=-3.0)),  # one non-default kwarg
        ),
    ),
    # Addition again but with two default kwargs.
    (
        lambda x=1.0, y=1.0: x + y,
        (
            (tuple(), dict()),  # no args
            ((-1.0,), dict(y=1.0)),  # one arg one kwarg
            (tuple(), dict(x=-3.0)),  # one non-default kwarg
...
compound_op_scalar_ufunc_dunder_fns = [
    # gaussian pdf
    (
        lambda x, mu, sigma: np.exp(-0.5 * ((x - mu) / sigma) ** 2.0)
    ...
    # polynomial
    (
        lambda x, a, b, c: a * x**2.0 + b * x + c,
        (
            ((0.0, 1.0, 2.0, 3.0), dict()),
            ((1.0, 1.0, 1.0, 1.0), dict()),
            ((-1.0, 1.0, -1.0, 1.0), dict()),
        ),
    ),
    # logistic
    (
        lambda x, a, b, c: c / (1.0 + np.exp(-a * (x - b))),
        (
            ((0.0, 1.0, 2.0, 3.0), dict()),
            ((1.0, 1.0, 1.0, 1.0), dict()),
            ((-1.0, 1.0, -1.0, 1.0), dict()),
...
def _reshape_argskwargs_elementwise(
    argskwargs_fns, shapes=((2,), (2, 1), (2, 2), (3, 2))
):
...
def _reshape_argskwargs_broadcast(argskwargs_fns):
    ...
    for f, argskwargs in argskwargs_fns:
        ...
        for args, kwargs in argskwargs:
            ...
            new_args = [
                _scalar_to_array(arg, _set_dim_to_1(i, end_shape))
                for i, arg in enumerate(args)
            ]
            new_kwargs = {
                k: _scalar_to_array(v, _set_dim_to_1(i + num_args, end_shape))
...
reduction_op_array_fns = [
    ...
    # )),
    # np.sum
    (lambda x: np.sum(x, axis=0), (((_a2r,), dict()),)),
    (lambda x: np.sum(x, axis=(0, 1)), (((_a2r,), dict()),)),
    (lambda x: np.sum(x, axis=None), (((_a2r,), dict()),)),
    # np.prod
    (lambda x: np.prod(x, axis=0), (((_a2r,), dict()),)),
    (lambda x: np.prod(x, axis=(0, 1)), (((_a2r,), dict()),)),
    (lambda x: np.prod(x, axis=None), (((_a2r,), dict()),)),
    ...
    # )),
    # @ matmul operator
    (lambda x: x @ x.T, (((_a2r,), dict()),)),
    # np.einsum
    (lambda x: np.einsum("ij->j", x), (((_a2r,), dict()),)),
    # TODO ...
]
# </Reduction Ops>
